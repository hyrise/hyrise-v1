...
class PointerCalcTests : public StorageManagerTest {
 protected:
  std::string table_7 =
      "Mobile | ID | Name | Mail | Company | Phone | Org\nINTEGER | INTEGER | INTEGER | INTEGER | INTEGER | INTEGER | "
...
atable_ptr_t createTable(std::string description) {
  ...
  params.setInput(io::CSVInput("test/tables/10_col_only_data.tbl",
                               io::CSVInput::params().setUnsafe(true).setCSVParams(io::csv::HYRISE_FORMAT)));
