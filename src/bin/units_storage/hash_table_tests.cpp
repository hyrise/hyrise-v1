...
::testing::AssertionResult TestCoverage(const atable_ptr_t& table, const field_list_t& columns) {
  HT ht(table, columns);
...
template <typename HT>
bool testHashTableFullCoverage(const HT& hashTable, const atable_ptr_t& table, const field_list_t& columns) {
  bool result = true;
...
class SingleHashTableTest : public ::hyrise::Test {
 public:
  ...
  virtual void SetUp() { table = io::Loader::shortcuts::load("test/join_exchange.tbl"); }
};
...
class HashTableViewTest : public ::hyrise::Test {
 protected:
  ...
  virtual void SetUp() { table = io::Loader::shortcuts::load("test/tables/hash_table_test.tbl"); }
};
...
TYPED_TEST(HashTableViewTest, full_range) {
  ...
  for (auto itMap = hashTab->getMapBegin(); itMap != hashTab->getMapEnd(); ++itMap) {
    auto key = itMap->first;
    EXPECT_TRUE(!view->get(key).empty());
  }
}
...
TYPED_TEST(HashTableViewTest, partial_range) {
  ...
  for (; hashPosition != hashEnd; std::advance(hashPosition, 1)) {
