...
class GetSetTests : public AccessTest {
 protected:
  ...
  GetSetTests() : AccessTest(), _table(io::Loader::shortcuts::load("test/empty.tbl")) {}
};

TEST_F(GetSetTests, basic_set) {
  SetTable st("new_table");
  st.addInput(_table);
  ASSERT_EQ(_table, st.execute()->getResultTable()) << "Operation result should be the input table";
