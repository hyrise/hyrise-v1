...
bool check_equality(const std::shared_ptr<AbstractHashTable> & ht1, const std::shared_ptr<AbstractHashTable> & ht2){
  bool isEqual = true;
...
TEST_F(HashBuildTest, check_equality) {
  ...
  hb.setKey("groupby");
  hb.execute();
  auto hash = std::dynamic_pointer_cast<AggregateHashTable >(hb.getResultHashTable());
  ...
  hb1.setKey("groupby");
  hb1.execute();
  auto hash1 = std::dynamic_pointer_cast<AggregateHashTable >(hb1.getResultHashTable());
  ...
  hb2.setKey("groupby");
  hb2.execute();
  auto hash2 = std::dynamic_pointer_cast<AggregateHashTable >(hb2.getResultHashTable());
...
TEST_F(HashBuildTest, merge_one_table_test) {
  ...
  hb.setKey("groupby");
  hb.execute();
  auto hash1 = std::dynamic_pointer_cast<AggregateHashTable >(hb.getResultHashTable());

  MergeHashTables mht;
  mht.addInputHash(hash1);
  mht.setKey("groupby");
  mht.execute();
  auto hash2 = std::dynamic_pointer_cast<AggregateHashTable >(mht.getResultHashTable());
...
TEST_F(HashBuildTest, merge_two_tables_test) {
  ...
  mht.setKey("groupby");
  mht.execute();
  auto hash3 = std::dynamic_pointer_cast<AggregateHashTable >(mht.getResultHashTable());
