...
typedef struct { std::uint64_t start; std::uint64_t stop; } d_result_t;
typedef struct { int rows; int part; int count; } d_params_t;
typedef struct { d_result_t expected_result; d_params_t params; std::string info; } test_param;


class ParallelExecutionTest : public testing::TestWithParam<test_param> {};

std::vector<test_param> tests = {
  { {0u, 1000u}, {2000, 0, 2}, "First partition"},
...
INSTANTIATE_TEST_CASE_P(ParallelExecutionTests,
