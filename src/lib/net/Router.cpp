...
RouterException::RouterException(const std::string &message)
...
const Router::route_map_t &Router::getRouters() const {
  return _route;
}

void Router::addRoute(const std::string &url,
                      handler_uptr handler,
                      route_t route,
                      const std::string &handler_name) {
  if (url == "/")
    throw RouterException("Registering for '/' is not possible, use " \
...
Router::handler_ptr Router::getCatchAllRaw() const {
  return _catch_all;
}

std::string Router::getHandlerNameForRoute(const std::string &url) const {
  return _route_names.at(url);
}
