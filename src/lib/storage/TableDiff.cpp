...
class EqualValueFunctor {
 ...
 public:
  ...
  EqualValueFunctor(const AbstractTable* const left,
                    ...
                    const size_t& line_left,
                    const size_t& line_right,
                    const field_t& col_left,
                    const field_t& col_right)
      : _left(left),
        _right(right),
        _line_left(line_left),
        _line_right(line_right),
        _col_left(col_left),
        _col_right(col_right) {}
...
bool line_equal(const AbstractTable* const left,
  ...
  for (field_t col_left = 0; col_left < left->columnCount(); ++col_left) {
    ...
    EqualValueFunctor ev(left, right, line_left, line_right, col_left, col_right);
...
std::map<field_t, field_t> compareSchemas(const AbstractTable* const left, const AbstractTable* const right) {
...
bool TableDiff::compare(const TableDiff::RelationEqType eqType) const {
  switch (eqType) {
    case TableDiff::RelationEq:
      return equal();
    case TableDiff::RelationEqSorted:
      return equalSorted();
  }
  return false;  // connot happen but there is a compiler warning
}
bool TableDiff::equal() const { return rowsMatched() && schemasMatched(); }
bool TableDiff::equalSorted() const { return rowsMatchedSorted() && schemasMatched(); }
...
bool TableDiff::rowsMatchedSorted() const { return wrongRows.empty() && falsePositionRows.empty(); }
bool TableDiff::rowsMatched() const { return wrongRows.empty(); }
...
TableDiff TableDiff::diffTables(const AbstractTable* const left,
  ...
  std::vector<bool> seen_right(right->size(), false);
