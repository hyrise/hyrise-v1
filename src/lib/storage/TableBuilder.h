...
#define builder_param_member(type, name) \
  type name;                             \
  param& set_##name(type t) {            \
    name = t;                            \
    return *this;                        \
  }
  ...
  */
  struct param_list {
    ...
    */
     ...
     */
    const param_list_t& params() const { return _params; }

    const group_list_t& groups() const { return _groups; }

    size_t size() const { return _params.size(); }

    const param_t& operator[](const size_t index) { return _params[index]; }
