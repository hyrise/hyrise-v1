...
class BitCompressedVector : public BaseAttributeVector<T> {
 ...
 public:
  ...
  BitCompressedVector(size_t columns, size_t rows, std::vector<uint64_t> bits = {})
      : _data(nullptr), _size(0), _allocatedBlocks(0), _columns(columns), _bits(bits) {
    // When bits is unset, behave like a fixed length vector
    if (bits.size() == 0) {
      _bits = std::vector<uint64_t>(_columns, sizeof(T) * 8);
    }
    reserve(rows);
  }
  ...
  inline uint64_t capacity() { return (_allocatedBlocks * _bit_width) / _tupleWidth(); }
 ...
 private:
...
#ifdef EXPENSIVE_ASSERTIONS
    if (column >= _columns) {
      throw std::out_of_range("Trying to access column '" + std::to_string(column) + "' where only '" +
                              std::to_string(_columns) + "' available");
    }
    if (rows >= _size) {
      throw std::out_of_range("Trying to access rows '" + std::to_string(rows) + "' where only '" +
   ...
   */
  inline uint64_t _blockPosition(uint64_t row) const { return (_tupleWidth() * row) / _bit_width; }

  /*
    Calculates the offset of the row inside the block
   */
  inline uint64_t _blockOffset(uint64_t row) const { return (_tupleWidth() * row) % _bit_width; }

  // Return the number of required blocks for the number of rows given
  // in the parameter
  inline uint64_t _blocks(uint64_t rows) const { return ((rows * _tupleWidth()) + _bit_width - 1) / _bit_width; }
