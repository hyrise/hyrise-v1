...
struct MapValueForValueId {
  ...
  void prepare(atable_ptr_t m,
               field_t dst,
               std::shared_ptr<AbstractDictionary> d,
               size_t col,
               std::shared_ptr<const SimpleStore::delta_table_t> de) {
    _main = m;
    _dstCol = dst;
    _dict = d;
    _delta = de;
...
void SimpleStoreMerger::mergeValues(const std::vector<c_atable_ptr_t>& input_tables,
  ...
  if (input_tables.size() != 2)
    throw std::runtime_error("SimpleStoreMerger does not support more than two tables");
