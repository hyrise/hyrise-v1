...
class DictionaryIterator : public boost::iterator_facade<DictionaryIterator<T>, T, boost::forward_traversal_tag> {
 ...
 public:
  ...
  T& dereference() const { return _it->dereference(); }

  bool equal(const DictionaryIterator& other) const { return _it->equal(other._it); }

  value_id_t getValueId() const { return _it->getValueId(); }
};
}
