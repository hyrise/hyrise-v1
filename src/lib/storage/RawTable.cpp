...
RowHelper::RowHelper(const metadata_vec_t& m) : _m(m) { _tempData.resize(m.size(), nullptr); }

RowHelper::~RowHelper() { reset(); }
...
RawTable::RawTable(const metadata_vec_t& m, size_t initial_size) : _metadata(m), _width(m.size()), _size(0) {
...
RawTable::~RawTable() { free(_data); }

size_t RawTable::size() const {
  return _size;
};
...
const ColumnMetadata& RawTable::metadataAt(const size_t column, const size_t row) const { return _metadata.at(column); }
...
table_id_t RawTable::subtableCount() const { return 1; }

atable_ptr_t RawTable::copy() const { return nullptr; }
...
struct convert_to_string_functor {
  ...
  convert_to_string_functor(const RawTable& table, const size_t& col, const size_t& row)
...
RawTable::byte* RawTable::getRow(size_t index) const {
  if (index >= _size)
    throw std::out_of_range("Index out of range for getRow()");
  byte* data = _data;
...
struct type_func {
  ...
  type_func(const atable_ptr_t& source, rawtable::RowHelper& rh, const size_t& column, const size_t& row)
      : _source(source), _rh(rh), _col(column), _row(row) {}
