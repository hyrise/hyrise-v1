...
std::vector<hyrise::storage::atable_ptr_t> TableMerger::mergeToTable(hyrise::storage::atable_ptr_t dest,
                                                                     std::vector<hyrise::storage::c_atable_ptr_t > &input_tables) const {
  // at least two tables
  ...
  if (tables.tables_to_merge.size() > 0) {
    ...
    // do the merge
    auto newSize = _strategy->calculateNewSize(tables.tables_to_merge);
    _merger->mergeValues(tables.tables_to_merge, dest, mapping, newSize);
...
std::vector<hyrise::storage::atable_ptr_t > TableMerger::merge(std::vector<hyrise::storage::c_atable_ptr_t > &input_tables) const {
  ...
  if (tables.tables_to_merge.size() > 1) {
    ...
    size_t new_size = _strategy->calculateNewSize(tables.tables_to_merge);
    ...
    // do the merge
    _merger->mergeValues(tables.tables_to_merge, merged_table, identityMap(merged_table), new_size);
