...
class VolatileAttributeVector : public FixedLengthVector<T> {
 ...
 public:
  ...
  void* data() { return _values; }

  void setNumRows(size_t s) { _rows = s; }
  ...
  void reserve(size_t rows) { allocate(_columns * rows * sizeof(T)); }
  ...
  inline uint64_t capacity() { return _allocated_bytes / (sizeof(value_type) * _columns); }

  std::shared_ptr<BaseAttributeVector<T>> copy() {
    auto copy = std::make_shared<VolatileAttributeVector<T>>(_columns, _rows);
    copy->_rows = _rows;
