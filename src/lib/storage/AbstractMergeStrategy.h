...
typedef struct _merge_tables {
  ...
  _merge_tables(std::vector<c_atable_ptr_t> _tables_to_merge, std::vector<c_atable_ptr_t> _tables_not_to_merge)
...
class AbstractMergeStrategy {
 public:
  ...
  virtual size_t calculateNewSize(const std::vector<c_atable_ptr_t>& input_tables,
                                  bool useValid,
                                  const std::vector<bool>& valid) const = 0;
...
class DefaultMergeStrategy : public AbstractMergeStrategy {
 ...
 public:
  ...
  virtual AbstractMergeStrategy* copy() { throw std::runtime_error("Merge Strategy Copy is like killing a kitten"); }
};
}
