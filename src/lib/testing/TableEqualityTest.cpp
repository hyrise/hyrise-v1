...
std::string rowErrors(TableDiff diff, const char* baseRelationName, const char* otherRelationName) {
  ...
  buf << "rows in \"" << baseRelationName << "\" that are not in \"" << otherRelationName << "\": ";
...
std::string rowPositionErrors(TableDiff diff, const char* baseRelationName, const char* otherRelationName) {
  ...
  buf << "rows in \"" << baseRelationName << "\" that are not in the same place as in\"" << otherRelationName << "\": ";
...
::testing::AssertionResult RelationEquals(const char* left_exp,
  ...
  buf << "\"" << left_exp << "\""
      ...
      << "\"" << right_exp << "\":" << std::endl << schemaErrors(resultL2R, left_exp, left)
      << schemaErrors(resultR2L, right_exp, right);
  if (!schema_only) {
    buf << rowErrors(resultL2R, left_exp, right_exp) << rowErrors(resultR2L, right_exp, left_exp);
  }
...
::testing::AssertionResult RelationNotEquals(const char* left_exp,
  ...
  if (resultL2R.equal() && resultR2L.equal())
    return ::testing::AssertionFailure() << left_exp << " and " << right_exp << " are not expected to be equal";

  return ::testing::AssertionSuccess();
}

::testing::AssertionResult SchemaEquals(const char* left_exp, const char* right_exp, tblptr left, tblptr right) {
...
::testing::AssertionResult SortedRelationEquals(const char* left_exp,
  ...
  buf << "\"" << left_exp << "\""
      ...
      << "\"" << right_exp << "\"" << std::endl << schemaErrors(resultL2R, left_exp, left)
      << schemaErrors(resultR2L, right_exp, right) << rowErrors(resultL2R, left_exp, right_exp)
      << rowErrors(resultR2L, right_exp, left_exp) << rowPositionErrors(resultL2R, left_exp, right_exp);
