...
void print(std::vector<std::vector<unsigned> > l) {
  ...
  BOOST_FOREACH(std::vector<unsigned> x, l) {
    BOOST_FOREACH(unsigned e, x) { std::cout << e << " "; }
    std::cout << "|";
  }
  std::cout << "]" << std::endl;
  ;
...
void print(std::vector<double> d) {
  BOOST_FOREACH(double e, d) { std::cout << e << " "; }
  std::cout << std::endl;
}
}
...
std::vector<subset_t>& operator+=(std::vector<subset_t>& a, const std::vector<subset_t>& b) {
  BOOST_FOREACH(subset_t i, b) { a.push_back(i); }
  return a;
}

std::vector<subset_t> operator+(const std::vector<subset_t>& a, const std::vector<subset_t>& b) {
  std::vector<subset_t> result;
  BOOST_FOREACH(subset_t i, a) { result.push_back(i); }
  BOOST_FOREACH(subset_t i, b) { result.push_back(i); }
  return result;
}
...
bool operator!=(const std::vector<subset_t>& left, const std::vector<subset_t>& right) { return !(left == right); }

bool sort_subset_by_size(const subset_t& left, const subset_t& right) { return left.size() > right.size(); }
