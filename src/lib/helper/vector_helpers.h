...
auto sum(T values, U initial, F func) -> U {
  ...
  return std::accumulate(values.begin(), values.end(), initial, [func](U l, VT v)->U { return l + func(v); });
}
...
std::vector<std::shared_ptr<T>> convert(const std::vector<std::shared_ptr<U>>& source) {
  return hyrise::functional::collect(
      source, [](decltype(*std::begin(source)) item) { return std::dynamic_pointer_cast<T>(item); });
}
...
auto getOrDefault(const MapType& map, typename MapType::key_type key, typename MapType::mapped_type def)
