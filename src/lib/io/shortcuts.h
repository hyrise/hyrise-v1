...
*/
std::shared_ptr<storage::AbstractTable> loadWithHeader(const std::string& datafilepath,
                                                       const std::string& headerfilepath);

/*! Load a table from a given filename, while supplying the header from the second input parameter. Must be HYRISE
  formatted.
  @param datafilepath
  @param header
*/
std::shared_ptr<storage::AbstractTable> loadWithStringHeader(const std::string& datafilepath,
...
/*! Load store with main and delta from separate files
  ...
  @param deltafilepath
*/
std::shared_ptr<storage::Store> loadMainDelta(const std::string& mainfilepath,
