...
void genericLineBasedParsing(std::istream& file,
  ...
  if (!csv_init(&parser, 0)) {
    ...
    while (std::getline(file, line)) {
      ...
      ++lineCount;
      line.append("\n");
      if (csv_parse(&parser, line.c_str(), line.size(), cb_per_field, cb_per_line, data) != line.size()) {
    ...
    csv_fini(&parser, cb_per_field, cb_per_line, data);
  }
  csv_free(&parser);
}
...
void genericParse(
  ...
  if (!csv_init(&parser, 0)) {
    ...
    do {
      ...
      if (csv_parse(&parser, rdbuf, readBytes, cb_per_field, cb_per_line, data) != (size_t)readBytes) {
        throw ParserError(csv_strerror(csv_error(&parser)));
      }
...
void genericParseFile(const std::string& filename,
                      field_cb_t cb_per_field,
                      line_cb_t cb_per_line,
                      void* data,
                      const csv::params& params) {
...
void vector_cb_per_line(int separator, struct vector_cb_data* data) { data->lines.resize(data->lines.size() + 1); }
