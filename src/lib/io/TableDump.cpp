...
struct write_to_stream_functor {
  ...
  write_to_stream_functor(std::ofstream& o, std::shared_ptr<AbstractTable> t) : data(o), table(t), col(0), vid(0) {}

  inline void setVid(value_id_t v) { vid = v; }

  inline void setCol(field_t c) { col = c; }
...
struct write_to_dict_functor {
  ...
  write_to_dict_functor(std::ifstream& d, std::shared_ptr<AbstractTable> t, field_t c) : data(d), table(t), col(c) {}
