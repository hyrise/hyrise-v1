...
WSCoreBoundPriorityQueue::WSCoreBoundPriorityQueue(int core, WSCoreBoundPriorityQueuesScheduler* scheduler)
    : AbstractCoreBoundQueue(core), _allQueues(nullptr), _scheduler(scheduler) {}

void WSCoreBoundPriorityQueue::init() { launchThread(_core); }
...
void WSCoreBoundPriorityQueue::executeTask() {
  ...
  while (1) {
    ...
    if (task) {
      ...
      // run task
      // std::cout << "Running task " << task->vname() << "; hex " << std::hex << &task << std::dec << " on core " <<
      // _core<< std::endl;
      (*task)();
...
std::shared_ptr<Task> WSCoreBoundPriorityQueue::stealTasks() {
  ...
  if (_allQueues != nullptr) {
    ...
    if (number_of_queues > 1) {
      // steal from the next queue (we only check number_of_queues -1, as we do not have to check the queue taht wants
      // to steal)
      for (int i = 1; i < number_of_queues; i++) {
        ...
        if (task != nullptr) {
          // push(task);
          // std::cout << "Queue " << _core << " stole Task " <<  task->vname() << "; hex " << std::hex << &task <<
          // std::dec << " from queue " << i << std::endl;
          break;
        }
      }
    }
...
void WSCoreBoundPriorityQueue::push(std::shared_ptr<Task> task) {
  // mutex is bad! but apparently we need it, otherwise, threads do not know whether they can sleep, cause runqueue.size
  // may be incorrect if not synced
...
void WSCoreBoundPriorityQueue::refreshQueues() { _allQueues = _scheduler->getTaskQueues(); }
}
