...
template<class QUEUE> class WSCoreBoundQueue;
...
class WSCoreBoundQueue : 
  virtual public WSThreadLevelQueue<QUEUE>,
  virtual public CoreBoundQueue<QUEUE>{
...
public:
  WSCoreBoundQueue(WSThreadLevelQueuesScheduler<QUEUE> * scheduler, size_t core, size_t threads): ThreadLevelQueue<QUEUE>(threads), WSThreadLevelQueue<QUEUE>(scheduler, threads), CoreBoundQueue<QUEUE>(core, threads) {}
  ~WSCoreBoundQueue(){}
...
std::shared_ptr<Task> stealTasks() {
  	...
  	if(_numberOfOtherQueues > 0){
  	  ...
  	  for (size_t i = 0; i < _numberOfOtherQueues; i++) {
  	    //std::cout << "Workerhread tries to steal task from queue " << i<< "  scheduler status " << status <<  std::endl;
