...
void CoreBoundPriorityQueue::executeTask() {
  ...
  while (1) {
    ...
    if (task) {
      // set queue to _blocked as we run task; this is a simple mechanism to avoid that further tasks are pushed to this
      // queue if a long running task is executed; check WSSimpleTaskScheduler for task stealing queue
      _blocked = true;
...
void CoreBoundPriorityQueue::init() { launchThread(_core); }
