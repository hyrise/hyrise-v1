...
namespace {
bool registered = SharedScheduler::registerScheduler<CentralScheduler>("CentralScheduler");
}
 ...
 */
size_t CentralScheduler::getNumberOfWorker() const { return _worker_threads.size(); }

/*
 * notify scheduler that a given task is ready
 */
void CentralScheduler::notifyReady(std::shared_ptr<Task> task) { _runQueue.push(task); }
}
