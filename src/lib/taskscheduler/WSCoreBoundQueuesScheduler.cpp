...
namespace {
bool registered = SharedScheduler::registerScheduler<WSCoreBoundQueuesScheduler>("WSCoreBoundQueuesScheduler");
}

WSCoreBoundQueuesScheduler::WSCoreBoundQueuesScheduler(const int queues) : AbstractCoreBoundQueuesScheduler(queues) {}
...
void WSCoreBoundQueuesScheduler::pushToQueue(std::shared_ptr<Task> task) {
  ...
  if (core >= 0 && core < static_cast<int>(this->_queues)) {
    ...
    if (core < Task::NO_PREFERRED_CORE || core >= static_cast<int>(this->_queues))
      ...
      LOG4CXX_WARN(
          this->_logger,
          "Tried to assign task " << std::hex << (void*)task.get() << std::dec << " to core " << std::to_string(core)
