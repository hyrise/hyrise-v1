...
void MultiplyRefField::executeMultiply() {
	...
	register const auto ref = _field_definition[0];
	register const auto val = _field_definition[1];
	...
	for(size_t outer=0; outer < stop; ++outer) {
		register auto tmp = itab->getValue<T>(val, outer);
		register auto col = itab->getValue<hyrise_int_t>(ref, outer);
	...
	std::vector<hyrise::storage::atable_ptr_t > vc ({left, result});
	hyrise::storage::atable_ptr_t tt = std::make_shared<MutableVerticalTable>(vc);
	addResult(tt);
