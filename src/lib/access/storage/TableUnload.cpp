...
void TableUnload::executePlanOperation() { io::StorageManager::getInstance()->removeTable(_table_name); }
...
const std::string TableUnload::vname() { return "TableUnload"; }

void TableUnload::setTableName(const std::string& tablename) { _table_name = tablename; }
}
}
