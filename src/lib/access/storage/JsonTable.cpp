...
void JsonTable::executePlanOperation() {
  ...
  if (rows > 0) {
    ...
    set_string_value_functor fun(_useStoreFlag ? std::dynamic_pointer_cast<storage::Store>(result)->getDeltaTable()
                                               : result);
    ...
    if (_useStoreFlag) {
      ...
      pos_list_t pl(rows);
      size_t counter = 0;
      std::generate(pl.begin(), pl.end(), [&counter]() { return counter++; });
...
void JsonTable::setData(const std::vector<std::vector<std::string>> d) { _data = d; }

void JsonTable::setNames(const std::vector<std::string> data) { _names = data; }

void JsonTable::setTypes(const std::vector<std::string> data) { _types = data; }

void JsonTable::setGroups(const std::vector<unsigned> data) { _groups = data; }

void JsonTable::setUseStore(const bool v) { _useStoreFlag = v; }

void JsonTable::setMergeStore(const bool v) { _mergeFlag = v; }
}
}
