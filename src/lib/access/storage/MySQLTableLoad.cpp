...
void MySQLTableLoad::executePlanOperation() {
  ...
  if (sm->exists(_table_name)) {
    ...
    t = io::Loader::load(io::Loader::params().setInput(io::MySQLInput(
        io::MySQLInput::params().setSchema(_database_name).setTable(_table_name).setLimit(_load_limit))));
...
const std::string MySQLTableLoad::vname() { return "MySQLTableLoad"; }

void MySQLTableLoad::setDatabaseName(const std::string& databaseName) { _database_name = databaseName; }

void MySQLTableLoad::setTableName(const std::string& tablename) { _table_name = tablename; }

void MySQLTableLoad::setLoadLimit(const uint64_t l) { _load_limit = l; }
}
}
