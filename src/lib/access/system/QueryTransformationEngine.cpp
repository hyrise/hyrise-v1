...
const std::string QueryTransformationEngine::parallelInstanceInfix = "_instance_",
                  QueryTransformationEngine::unionSuffix = "_union", QueryTransformationEngine::mergeSuffix = "_merge";
...
bool QueryTransformationEngine::requestsParallelization(Json::Value& operatorConfiguration) const {
  const bool parallelize = operatorConfiguration["instances"] >= 2;
  return parallelize;
}

void QueryTransformationEngine::applyParallelizationTo(Json::Value& operatorConfiguration,
  ...
  std::vector<std::string>* instanceIds =
      buildInstances(query, operatorConfiguration, operatorId, consolidateOperatorId);
  replaceOperatorWithInstances(operatorId, *instanceIds, consolidateOperatorId, query, numberOfInitialEdges);
  delete instanceIds;
}

std::vector<std::string>* QueryTransformationEngine::buildInstances(Json::Value& query,
                                                                    Json::Value& operatorConfiguration,
                                                                    const std::string& operatorId,
  ...
  for (size_t i = 0; i < numberOfInstances; ++i) {
    ...
    Json::Value nextInstance = nextInstanceOf(operatorConfiguration, operatorId, i, numberOfInstances, numberOfCores);
...
Json::Value QueryTransformationEngine::nextInstanceOf(Json::Value& operatorConfiguration,
                                                      const std::string& operatorId,
...
void QueryTransformationEngine::replaceOperatorWithInstances(const std::string& operatorId,
                                                             const std::vector<std::string>& instanceIds,
                                                             const std::string& consolidateOperatorId,
                                                             Json::Value& query,
                                                             const size_t numberOfInitialEdges) const {
  appendInstancesDstNodeEdges(operatorId, instanceIds, query, numberOfInitialEdges);
  appendConsolidateSrcNodeEdges(operatorId, instanceIds, consolidateOperatorId, query, numberOfInitialEdges);
...
void QueryTransformationEngine::removeOperatorNodes(Json::Value& query, const Json::Value& operatorId) const {
...
void QueryTransformationEngine::appendEdge(const std::string& srcId,
...
std::string QueryTransformationEngine::instanceIdFor(const std::string operatorId, const size_t instanceId) const {
  std::ostringstream s;
  s << instanceId;
  return operatorId + parallelInstanceInfix + s.str();
}

std::string QueryTransformationEngine::unionIdFor(const std::string& operatorId) const {
  return operatorId + unionSuffix;
}

std::string QueryTransformationEngine::mergeIdFor(const std::string& operatorId) const {
