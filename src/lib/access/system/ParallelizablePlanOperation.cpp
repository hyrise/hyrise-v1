...
std::pair<std::uint64_t, std::uint64_t> ParallelizablePlanOperation::distribute(const std::uint64_t numberOfElements,
...
void ParallelizablePlanOperation::splitInput() {
  ...
  if (_count > 0 && !tables.empty()) {
    ...
    input.setTable(
        storage::TableRangeView::create(std::const_pointer_cast<storage::AbstractTable>(tables[0]), r.first, r.second),
        0);
  }
}
...
void ParallelizablePlanOperation::setPart(size_t part) { _part = part; }
void ParallelizablePlanOperation::setCount(size_t count) { _count = count; }
}
}
