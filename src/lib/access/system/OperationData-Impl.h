...
std::shared_ptr<const T> OperationData::nthOf(size_t index) const {
  ...
  throw OperationDataError("Could not find resource indexed " + std::to_string(index) + " when only " + std::to_string(i) + " available");
}
...
void OperationData::setNthOf(size_t index, const std::shared_ptr<const T>& element) {
  ...
  throw OperationDataError("Could not set resource indexed " + std::to_string(index) + " when only " + std::to_string(i) + " available");
}
...
size_t OperationData::sizeOf() const {
  ...
  return std::count_if(std::begin(_resources), std::end(_resources), [] (const element_type& e) { return std::dynamic_pointer_cast<const T>(e) != nullptr; });
}
