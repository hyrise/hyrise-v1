...
struct QueryParserFactory<T, parse_construct> : public AbstractQueryParserFactory {
  ...
  virtual std::shared_ptr<PlanOperation> parse(const Json::Value& data) { return T::parse(data); }
};
...
class QueryParser {
  ...
  void buildTasks(const Json::Value& query,
                  std::vector<std::shared_ptr<taskscheduler::Task> >& tasks,
                  task_map_t& task_map) const;

  /*  Defines operations input based on their types.  */
  void setInputs(std::shared_ptr<PlanOperation> planOperation, const Json::Value& planOperationSpec) const;
  ...
  void setDependencies(const Json::Value& query, task_map_t& task_map) const;

  //  Output of task without successor is query's result.
  std::shared_ptr<taskscheduler::Task> getResultTask(const task_map_t& task_map) const;
 ...
 public:
  ...
  std::vector<std::shared_ptr<taskscheduler::Task> > deserialize(const Json::Value& query,
