...
void QueryParser::buildTasks(const Json::Value& query,
                             std::vector<std::shared_ptr<taskscheduler::Task> >& tasks,
  ...
  for (unsigned i = 0; i < members.size(); ++i) {
    ...
    std::shared_ptr<PlanOperation> planOperation = QueryParser::instance().parse(typeName, planOperationSpec);
...
void QueryParser::setInputs(std::shared_ptr<PlanOperation> planOperation, const Json::Value& planOperationSpec) const {
  //  TODO: input implies table input at this moment
  for (unsigned j = 0; j < planOperationSpec["input"].size(); ++j) {
    planOperation->addInput(io::StorageManager::getInstance()->getTable(planOperationSpec["input"][j].asString()));
  }
}
...
std::shared_ptr<taskscheduler::Task> QueryParser::getResultTask(const task_map_t& task_map) const {
  ...
  for (it = task_map.begin(); it != task_map.end(); ++it) {
    ...
    if (!currentTask->hasSuccessors() && it->first.asString() != autojsonReferenceTableId) {
