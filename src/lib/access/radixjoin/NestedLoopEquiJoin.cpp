...
void NestedLoopEquiJoin::executePlanOperation() {
  ...
  const auto& lprefixvector =
      std::dynamic_pointer_cast<storage::FixedLengthVector<value_id_t>>(lavsp.at(0).attribute_vector);
  ...
  const auto& rprefixvector =
      std::dynamic_pointer_cast<storage::FixedLengthVector<value_id_t>>(ravsp.at(0).attribute_vector);
...
const std::string NestedLoopEquiJoin::vname() { return "NestedLoopEquiJoin"; }

void NestedLoopEquiJoin::setBits1(const uint32_t b) { _bits1 = b; }

void NestedLoopEquiJoin::setBits2(const uint32_t b) { _bits2 = b; }

uint32_t NestedLoopEquiJoin::bits1() const { return _bits1; }

uint32_t NestedLoopEquiJoin::bits2() const { return _bits2; }

void NestedLoopEquiJoin::addPartition(const int p) { _partitions.push_back(p); }
}
}
