...
bool RadixJoinTransformation::transformation_is_registered =
    QueryTransformationEngine::registerTransformation<RadixJoinTransformation>();

void RadixJoinTransformation::appendEdge(const std::string& srcId, const std::string& dstId, Json::Value& query) const {
...
void RadixJoinTransformation::removeOperator(Json::Value& query, const Json::Value& operatorId) const {
  ...
  for (unsigned i = 0; i < query["edges"].size(); ++i) {
    ...
    if (currentEdge[0u] != jsonOperatorId && currentEdge[1u] != jsonOperatorId) {
...
ops_and_edges_t RadixJoinTransformation::build_probe_side(std::string prefix,
  ...
  for (int i = 0; i < probe_par; i++) {
    ...
    for (int j = 0; j < probe_par; j++) {
      probe_side.edges.push_back(
          createEdge(prefix + "_histogram_p1_" + std::to_string(i), prefix + "_prefixsum_p1_" + std::to_string(j)));
    }
    ...
    probe_side.edges.push_back(
        createEdge(prefix + "_createradixtable_p1", prefix + "_radixcluster_p1_" + std::to_string(i)));

    // From each prefix sum there is a link to radix clustering
    probe_side.edges.push_back(
        createEdge(prefix + "_prefixsum_p1_" + std::to_string(i), prefix + "_radixcluster_p1_" + std::to_string(i)));
...
ops_and_edges_t RadixJoinTransformation::build_hash_side(std::string prefix,
  ...
  Json::Value histogram_p1, prefixsum_p1, createradixtable_p1, radixcluster_p1, histogram_p2, prefixsum_p2,
      createradixtable_p2, radixcluster_p2, merge_prefix_sum, barrier;
  ...
  for (int i = 0; i < hash_par; i++) {
    ...
    for (int j = 0; j < hash_par; j++) {
      ...
      hash_side.edges.push_back(
          createEdge(prefix + "_radixcluster_p1_" + std::to_string(i), prefix + "_histogram_p2_" + std::to_string(j)));
      hash_side.edges.push_back(
          createEdge(prefix + "_histogram_p2_" + std::to_string(j), prefix + "_prefixsum_p2_" + std::to_string(i)));
    }
    // This builds up the second pass radix cluster, attention order matters
    hash_side.edges.push_back(
        createEdge(prefix + "_radixcluster_p1_" + std::to_string(i), prefix + "_radixcluster_p2_" + std::to_string(i)));
    hash_side.edges.push_back(
        createEdge(prefix + "_createradixtable_p2", prefix + "_radixcluster_p2_" + std::to_string(i)));
    hash_side.edges.push_back(
        createEdge(prefix + "_prefixsum_p2_" + std::to_string(i), prefix + "_radixcluster_p2_" + std::to_string(i)));
...
void RadixJoinTransformation::distributePartitions(const int partitions,
...
void RadixJoinTransformation::transform(Json::Value& op, const std::string& operatorId, Json::Value& query) {
  ...
  ops_and_edges_t probe_side =
      build_probe_side(operatorId + "_probe", probe_field, probe_par, bits1, bits2, input_edges[0]);
