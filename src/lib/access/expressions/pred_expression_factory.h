...
#define GENERATE_2VALUE_EXPRESSION(EXPRESSION)                                       \
  case PredicateType::EXPRESSION:                                                    \
    if (_field_name.size() > 0)                                                      \
      return new EXPRESSION<ValueType>(_input_index,                                 \
                                       _field_name,                                  \
                                       json_converter::convert<ValueType>(_value),   \
                                       json_converter::convert<ValueType>(_value2)); \
    else                                                                             \
      return new EXPRESSION<ValueType>(_input_index,                                 \
                                       _field,                                       \
                                       json_converter::convert<ValueType>(_value),   \
...
struct expression_factory {
  ...
  expression_factory(size_t input_index,
                     ...
                     Json::Value value,
                     Json::Value value2 = Json::Value())
      : _input_index(input_index), _field_name(fieldName), _predicate(predicate), _value(value), _value2(value2) {}
