...
#define GENERATE_EXPRESSION(EXPRESSION)                                                                        \
...
#define GENERATE_EXPRESSION_OF_TYPE(EXPRESSION, TYPE)                                          \
  case PredicateType::EXPRESSION:                                                              \
    if (_field_name.size() > 0)                                                                \
...
#define GENERATE_EXPRESSION_WITH_VALUE_VECTOR(EXPRESSION)                  \
  case PredicateType::EXPRESSION:                                          \
    if (_field_name.size() > 0)                                            \
...
#define GENERATE_GENERIC_EXPRESSION(NAME, EXPRESSION, OPERATOR)                   \
  case PredicateType::NAME:                                                       \
    if (_field_name.size() > 0)                                                   \
      return new EXPRESSION<ValueType, OPERATOR<ValueType> >(                     \
          _input_index, _field_name, json_converter::convert<ValueType>(_value)); \
    else                                                                          \
      return new EXPRESSION<ValueType, OPERATOR<ValueType> >(                     \
          _input_index, _field, json_converter::convert<ValueType>(_value));      \
    break;
...
struct expression_factory {
  ...
  expression_factory(size_t input_index, field_t field_index, PredicateType::type predicate, Json::Value value)
      : _input_index(input_index), _field(field_index), _field_name(""), _predicate(predicate), _value(value) {}

  expression_factory(size_t input_index, field_name_t fieldName, PredicateType::type predicate, Json::Value value)
      : _input_index(input_index), _field_name(fieldName), _predicate(predicate), _value(value) {}
