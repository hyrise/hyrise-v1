...
class CompoundJoinExpression : public JoinExpression {
 ...
 public:
  ...
  CompoundJoinExpression(JoinExpression* _lhs, JoinExpression* _rhs, ExpressionType _type)
...
class EqualsJoinExpression : public JoinExpression {
 ...
 public:
  EqualsJoinExpression(size_t l, field_t _left, size_t r, field_t right)
      : left_field(_left), right_field(right), left_input(l), right_input(r) {}

  EqualsJoinExpression(size_t l, field_name_t _left, size_t r, field_name_t right)
      : _left_field_name(_left), _right_field_name(right), left_input(l), right_input(r) {}
  ...
  virtual void walk(const std::vector<storage::c_atable_ptr_t>& i) {
    ...
    if (_left_field_name.size() > 0)
      left_field = left->numberOfColumn(_left_field_name);
    if (_right_field_name.size() > 0)
      right_field = right->numberOfColumn(_right_field_name);
  }
