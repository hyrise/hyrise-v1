...
class SimpleFieldExpression : public SimpleExpression {
 ...
 public:
  ...
  SimpleFieldExpression(storage::c_atable_ptr_t table, field_t field_index)
      : table(table), field(field_index), input(0) {}

  SimpleFieldExpression(size_t input_index, field_name_t field_name)
      : field(0), field_name(field_name), input(input_index) {}

  SimpleFieldExpression(storage::c_atable_ptr_t table, field_name_t field_name)
      : table(table), field(0), field_name(field_name), input(0) {}
...
class GenericExpressionValue : public SimpleFieldExpression {
 ...
 public:
  ...
  inline virtual bool operator()(size_t row) { return _operator(table->template getValue<T>(field, row), value); }
};
}
