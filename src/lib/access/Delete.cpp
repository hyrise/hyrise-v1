...
void DeleteOp::executePlanOperation() {
  ...
  auto store =
      std::const_pointer_cast<storage::Store>(checked_pointer_cast<const storage::Store>(tab->getActualTable()));
...
std::shared_ptr<PlanOperation> DeleteOp::parse(const Json::Value& data) { return std::make_shared<DeleteOp>(); }
}
}
