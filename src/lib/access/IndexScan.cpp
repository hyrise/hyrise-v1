...
struct CreateIndexValueFunctor {
  ...
  const rapidjson::Value &_d;

  explicit CreateIndexValueFunctor(const rapidjson::Value &c): _d(c) {}
...
std::shared_ptr<PlanOperation> IndexScan::parse(const rapidjson::Value &data) {
...
std::shared_ptr<PlanOperation> MergeIndexScan::parse(const rapidjson::Value &data) {
