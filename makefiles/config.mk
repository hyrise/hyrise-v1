...
ifeq ($(PERSISTENCY),SIMPLELOGGER)
	BUILD_FLAGS += -D PERSISTENCY_SIMPLELOGGER
else ifeq ($(PERSISTENCY),BUFFEREDLOGGER)
	BUILD_FLAGS += -D PERSISTENCY_BUFFEREDLOGGER
else ifeq ($(PERSISTENCY),NVRAM)
	BUILD_FLAGS += -D PERSISTENCY_NVRAM
	BUILD_FLAGS += -D NVRAM_MOUNTPOINT=\"$(NVRAM_MOUNTPOINT)\"
	BUILD_FLAGS += -D NVRAM_FOLDER=\"$(NVRAM_FOLDER)\"
	BUILD_FLAGS += -D NVRAM_FILENAME=\"$(NVRAM_FILENAME)\"
	BUILD_FLAGS += -D NVRAM_FILESIZE=$(NVRAM_FILESIZE)ul
	BUILD_FLAGS += -D NVSIMULATOR_FLUSH_NS=$(NVSIMULATOR_FLUSH_NS)
	BUILD_FLAGS += -D NVSIMULATOR_READ_NS=$(NVSIMULATOR_READ_NS)
	BUILD_FLAGS += -D NVSIMULATOR_WRITE_NS=$(NVSIMULATOR_WRITE_NS)
else
	BUILD_FLAGS += -D PERSISTENCY_NONE
endif
